...
public partial class ImportManager : IImportManager
{
    ...
    public virtual async Task ImportProductsFromXlsxAsync(Stream stream)
    {
        ...
        try
        {
            ...
            {
                if (!await _categoryService.CanVendorAddProductsAsync(category, allCategoryList))
                    continue;

                var keyName = await _categoryService.GetFormattedBreadCrumbAsync(category, allCategoryList);

                allCategories.Add(new CategoryKey(keyName, category, category.LimitedToStores ? (await _storeMappingService.GetStoresIdsWithAccessAsync(category)).ToList() : new List<int>()), category);
            }
        }
